...
interface EvolutionUniversitySelectorProps {
  allUniversities: UniversityData[];
  selectedNames: string[];
  onSelectionChange: (name: string) => void;
}

const EvolutionUniversitySelector: React.FC<EvolutionUniversitySelectorProps> = ({ allUniversities, selectedNames, onSelectionChange }) => {
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      >
        <h2 className="text-xl font-bold text-slate-800">
          Seleccionar Universidades para la Evolución ({selectedNames.length}/{MAX_SELECTIONS})
        </h2>
...
export default EvolutionUniversitySelector;